...
try {
	...
	switch ($_POST['handle']) {
		case 'cover-user':
			/* update user cover */
			$db->query(sprintf("UPDATE users SET user_cover = '', user_cover_id = '' WHERE user_id = %s", secure($user->_data['user_id'], 'int') )) or _error(SQL_ERROR_THROWEN);
			break;

		case 'picture-user':
			/* update user picture */
			$db->query(sprintf("UPDATE users SET user_picture = '', user_picture_id = '' WHERE user_id = %s", secure($user->_data['user_id'], 'int') )) or _error(SQL_ERROR_THROWEN);
			/* return */
		...
		case 'cover-page':
			...
			/* update page cover */
			$db->query(sprintf("UPDATE pages SET page_cover = '', page_cover_id = '' WHERE page_id = %s", secure($_POST['id'], 'int') )) or _error(SQL_ERROR_THROWEN);
			break;
		...
		case 'picture-page':
			...
			/* update page picture */
			$db->query(sprintf("UPDATE pages SET page_picture = '', page_picture_id = '' WHERE page_id = %s", secure($_POST['id'], 'int') )) or _error(SQL_ERROR_THROWEN);
			/* return */
		...
		case 'cover-group':
			...
			/* update group cover */
			$db->query(sprintf("UPDATE groups SET group_cover = '', group_cover_id = '' WHERE group_id = %s", secure($_POST['id'], 'int') )) or _error(SQL_ERROR_THROWEN);
			break;
		...
		case 'picture-group':
			...
			$db->query(sprintf("UPDATE groups SET group_picture = '', group_picture_id = '' WHERE group_id = %s", secure($_POST['id'], 'int') )) or _error(SQL_ERROR_THROWEN);
			/* return */
